  ...
  handleSubmit = async (values) => {
    ...
    const filteredValues = Object.keys(values).reduce((acc, key) => {
      if (
        ...
        selectedOptionSchema?.properties.hasOwnProperty(key) // Include only fields present in the schema
      ) {
        acc[key] = values[key];
      }
      return acc;
    ...
    try {
      ...
      addNotification({
        ...
        content: i18next.t("Job {{title}} has been scheduled.", { id: data.title }),
        type: "success",
      });
          ...
          <Form>
              <Modal.Content>
                <Dropdown
                  ...
                  options={options}
                  className="mb-10"
                  onChange={(e, { value }) => setFieldValue("selectedOption", value)}
                  value={values.selectedOption}
                />
                {values.selectedOption && payloadSchema[values.selectedOption] && (
                  <>
                    {Object.keys(payloadSchema[values.selectedOption].properties).map((property) => (
                      <Field
                        key={property}
                        name={property}
                        render={({ field }) => (
                          <Input
                            {...field}
                            label={payloadSchema[values.selectedOption].properties[property].metadata?.title}
                            className="m-5"
                            type={payloadSchema[values.selectedOption].properties[property].type === "string" ? "text" : "number"}
                          />
                        )}
                      />
                    ))}
